...
//编译目录
def BUILD_DIR = "ims-api/${PROJECT_NAME}/"

//相对编译目录的jar包路径 
def JAR_PATH = "target/${PROJECT_NAME}.jar"
//目标主机 jar包路径
def DEST_HOST_JAR_DIR = "/home/java/${PROJECT_NAME}/"

//
def ACTIVE_NAME = "test"
...
//定义端口
if ( PROJECT_NAME == "app-0" ) {
    SERVER_PORT="9003"
} else if ( PROJECT_NAME == "app-1" ){
    SERVER_PORT="9002"
} else if ( PROJECT_NAME == "app-2" ){
    SERVER_PORT="8004"
} else if ( PROJECT_NAME == "app-3" ){
    SERVER_PORT="9001"
} else if ( PROJECT_NAME == "app-4" ){
    SERVER_PORT="9005"
} else if ( PROJECT_NAME == "app-4" ){
    SERVER_PORT="9008"
}
node{
    stage('拉代码'){
       git branch: "${BRANCH}", credentialsId: 'xxxx', url: 'http://xxxxx/xxx.git'
    }
   ...
   //定义mvn环境
    ...
    stage('编译'){
        //mvn构建
        sh "cd ${BUILD_DIR} && mvn clean install -Dmaven.test.skip=true"
    }
    ...
    stage('复制包到目标主机'){
        //复制jar包
		sh "cd ${BUILD_DIR} && scp -P${SSH_PORT} -i /home/id_rsa  ${JAR_PATH}  ${SSH_USER}@${DEST_HOST}:${DEST_HOST_JAR_DIR}"
    }
	...
	stages {
		stage('拉代码'){
			when {
				...
				steps{
					git branch: "${BRANCH}", credentialsId: 'springjava', url: 'http://xxxx/xxx.git'
				}
		}
...
~~~
def credentialsId = "front"
def SSH_USER = "centos"
def SSH_PORT = "61122"
DEST_HOST_DIR = "/usr/local/nginx/html/pc"

//配置不同的git地址
if ( GIT_URI == "PC" ) {
    GIT_URL="http://xxxxx/xxxx.git"
} else if ( GIT_URI == "H5" ){
    GIT_URL="http://xxxxx/xxxx.git"
}
...
pipeline {
  ...
  stages {
    ...
    stage("编译代码"){
	   steps{
	     nodejs('nodejs12') {
		   sh"npm i --unsafe-perm && npm run build:rb"
		    }
		 }
	 }
    stage("打包复制到目标主机"){
	   steps{
		 sh """
			...
			ls -l && \
            scp -P${SSH_PORT} -i /home/id_rsa  dist.tar.gz  ${SSH_USER}@${DEST_HOST}:${DEST_HOST_DIR}/${GIT_URI}
		 """
		 }
	 }
    stage("远程执行脚本"){
	   steps{
		sh """
			ssh -p ${SSH_PORT} -i /home/id_rsa  ${SSH_USER}@${DEST_HOST} -tt << remotessh
			cd ${DEST_HOST_DIR}/${GIT_URI}
			rm -rf old-dist-${GIT_URI}
			mv dist old-dist-${GIT_URI}
			tar xf dist.tar.gz
			exit
			remotessh
		"""
...
~~~
    post {
        //任务失败执行
        failure {
          sh "wget --post-data 'content= 发布失败 请关注' url"
        }
        //任务成功后执行
        success {
          sh "wget --post-data 'content= ' url"
        }    
    }
~~~
